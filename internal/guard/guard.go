...
type Service struct {
	...
	audit   *audit.Service
	booths  map[string]domain.Booth
}

func NewService(st *store.Store, zoneSvc *zone.Service, hallSvc *hall.Service, auditSvc *audit.Service) *Service {
	svc := &Service{store: st, zoneSvc: zoneSvc, hallSvc: hallSvc, audit: auditSvc}
	halls, err := hallSvc.ListHalls()
	if err == nil {
		svc.booths = map[string]domain.Booth{}
		for _, hall := range halls {
			booths, err := hallSvc.ListBooths(hall.ID)
			if err == nil {
				for _, booth := range booths {
					svc.booths[booth.ID] = booth
				}
			}
		}
	}
